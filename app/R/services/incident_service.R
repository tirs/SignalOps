...
create_incident <- function(pool, tenant_id, anomaly_id = NULL, title, description = NULL,
                            severity = "medium", created_by, assigned_to = NULL, 
                            sla_hours = NULL) {
...
update_incident_status <- function(pool, incident_id, new_status, user_id, notes = NULL) {
  # Get current status
...
get_incident_stats <- function(pool, tenant_id) {
  stats <- safe_query(pool, "
    SELECT 
      ...
      COUNT(*) FILTER (WHERE sla_due_at < CURRENT_TIMESTAMP AND status NOT IN ('closed', 'false_positive')) as sla_breached_count,
      AVG(EXTRACT(EPOCH FROM (COALESCE(resolved_at, CURRENT_TIMESTAMP) - created_at)) / 3600) 
        FILTER (WHERE status IN ('closed', 'false_positive')) as avg_resolution_hours
    FROM incidents
    WHERE tenant_id = $1
  ", params = list(tenant_id))
